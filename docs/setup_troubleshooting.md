...
## Part 2.10 — Figure Styling: Theme Inheritance, Caption Placement, and Legend Artifacts

This section documents a longer iterative debugging arc on the same
phylogenetic tree figure, prompted by the figure not visually matching real
published journal figures despite being scientifically correct. It is kept
separate from Part 2.8/2.9 because the root causes here were different in
kind: not crashes, but silent styling failures and a genuine design
misunderstanding about what belongs inside an image file versus a
manuscript.

### Theme edit silently had no effect because of which theme function was called

**Symptom:** after editing `theme_publication()`'s `base_family` from
`"Arial"` to `"DejaVu Sans"` and later to `"Liberation Serif"`, the
rendered tree figure's text visibly remained in the original
(DejaVu-Sans-like) font with no change at all, despite `grep` confirming
the edit had genuinely landed in `theme_publication.R`.
**Cause:** `06_phylogeny.R` does not call `theme_publication()` — it calls
`theme_tree2()`, a separate, ggtree-specific theme function that has its
own independent default font handling and does not inherit from
`theme_publication()`'s `base_family` setting at all. Editing the wrong
theme function is silent: no error, just no visible effect, which made
this easy to mistake for "the fix doesn't work" rather than "the fix was
applied to code that isn't being used here."
**Fix:** pass `base_family` directly into the actual theme function being
used in that specific script:
```r
theme_tree2(base_size = 10, base_family = "Liberation Serif")
```
**Lesson:** in a project with multiple theme/style utility functions
(`theme_publication()`, `theme_tree2()`, any future custom theme), always
confirm *which specific theme call* a given script is actually using
before editing a *different* theme function and expecting the change to
propagate. `grep` for the theme function name actually invoked in the
failing script, not just the utility file you assume it's calling.

### A `theme()` override placed after a theme function does not override that theme's own explicit per-element settings

**Symptom:** added `theme(text = element_text(family = "Liberation Serif"))`
immediately after `theme_tree2(...)`, expecting this generic override to
apply the font everywhere — it did not change the rendered font at all.
**Cause:** `theme_tree2()` (like many ggplot2/ggtree theme functions) sets
explicit `family` values on *specific* elements (title, axis text, legend
text, etc.) rather than leaving them to inherit from the top-level `text`
element. A later generic `theme(text = ...)` call does not retroactively
override those more specific element-level settings — in ggplot2's theme
system, more specific element overrides win regardless of call order.
**Fix:** the only reliable fix was passing `base_family` as an argument
directly into `theme_tree2()` itself (see above), rather than attempting to
patch the font afterward via a generic `theme()` call.
**Lesson:** when a `theme()` call appears to have no effect on a specific
visual property (font, color, size), suspect that the base theme function
being layered on top of is setting that property explicitly at a more
specific element level — check the source/documentation of that base theme
function for an equivalent constructor argument (most ggplot2-ecosystem
theme functions accept `base_family`/`base_size` directly) rather than
trying to override it after the fact.

### Design error: embedding caption/title text inside the figure image itself

**Symptom (not a bug — a design correction prompted by user feedback):**
multiple iterations were spent trying to fix font rendering, encoding
(`\u2265` displaying as a box character), and styling of a title and a long
descriptive caption that were both rendered as text *inside* the saved
PNG/PDF, attempting to match how figures appear in published papers.
**Root issue:** this entire approach was based on a misunderstanding of
academic figure convention. In real papers, the figure file itself contains
**only the visual** (the plot, axes, legend, scale bar) — no title, no
"Figure N." label, no descriptive caption. That text is part of the
**manuscript document**, set in the document's own body font, and is placed
by the document/typesetting system below wherever the figure is inserted —
never baked into the image file.
**Resolution:** removed `ggtitle()` and `labs(caption = ...)` entirely from
the figure script. The script now outputs only the tree plot, color legend,
and scale bar. The "Figure 1. Maximum-likelihood phylogeny..." sentence and
its methods description instead belongs in the surrounding context wherever
the image is displayed (a markdown cell in a Jupyter notebook, the body
text of a report, or a manuscript's figure legend) — written once there,
not regenerated by the plotting script.
**Lesson:** before spending further engineering effort perfecting how text
renders *inside* a generated figure, check whether that text should be
inside the figure at all. This single realization eliminated an entire
category of recurring font/encoding bugs at once, since removing the text
removed the only thing in the figure that was ever vulnerable to those
bugs. It is also simply the correct academic convention: keep figure files
as pure visuals, and write captions once, in the surrounding document, in
that document's own typesetting system.

### Legend glyph artifact ("a"-shaped marks next to legend swatches)

**Symptom:** the color legend at the bottom of the figure showed a small
stray "a"-shaped (later, after one fix attempt, an empty box □) glyph
immediately to the left of each color swatch, alongside the correct color
dot and label text.
**Cause:** the plot used both `geom_tippoint()` (point) and `geom_tiplab()`
(text label) mapped to the same `color = state` aesthetic. ggplot2 merges
multiple geoms mapped to the same aesthetic into one combined legend key by
default, and that merged key was rendering an extra default text-glyph
contribution from `geom_tiplab()`'s key (normally an "a" placeholder
character ggplot2 uses internally to preview text geoms) layered behind the
point's color dot.
**Failed first attempt:** tried overriding the legend key glyph with an
explicit Unicode bullet character (`"\u25CF"`), which instead rendered as an
empty box, because that specific Unicode glyph was not present in the
`Liberation Serif` font's character set being used for legend text — a
related but distinct issue from the original artifact.
**Correct fix:** used `override.aes` inside `scale_color_manual()`'s
`guide_legend()` to explicitly suppress the unwanted text-key contribution
rather than trying to replace it with a different character:
```r
scale_color_manual(
  values = state_colors, name = NULL,
  guide = guide_legend(override.aes = list(label = "", linetype = 0))
)
```
Setting `label = ""` removes the stray text-glyph contribution from the
merged legend key entirely, leaving only the clean point/color swatch.
**Lesson:** when a legend shows an unexpected extra glyph and multiple
geoms share the same color aesthetic, the cause is very often legend-key
*merging* between those geoms rather than a problem with any single geom's
own settings. The fix is almost always an `override.aes` inside the
relevant `scale_*()` call's `guide_legend()`, explicitly suppressing the
unwanted contribution — not attempting to substitute a different glyph
character, which risks introducing a new, unrelated font-coverage problem
(as happened here on the first attempt).
...
| Editing one theme function's font has no visible effect | Script is calling a *different* theme function that doesn't inherit from it | `grep` the script for which theme call (`theme_publication`, `theme_tree2`, etc.) is actually used; pass `base_family` directly into that one |
| A later generic `theme(text=...)` doesn't override a font/color set earlier | The base theme function sets that property on a more specific element | Pass the override as a constructor argument to the base theme function itself, not via a later `theme()` patch |
| Figure text (title/caption) keeps having font/encoding bugs | Text may not belong inside the image file at all | Real journal figures contain only the visual; captions belong in the surrounding document, not baked into the PNG/PDF |
| Stray extra glyph appears next to a legend swatch | Two geoms mapped to the same aesthetic merged into one legend key | Use `override.aes = list(label = "", ...)` inside `guide_legend()` to suppress the unwanted merged contribution |
